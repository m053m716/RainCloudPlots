...
% DEFAULTS
pars = struct;
pars.Top = 0.025; % Offset (norm to height) of bottom border of graphical item [0, 1]
pars.Bot = 0.025; % Offset (norm to height) of graphical item [0, 1]
...
if abs(pars.Top - 1) > 1
   error(['RainCloudPlots:' mfilename ':ParameterOutOfBounds'],...
         ['\n\t\t->\t<strong>[UIGETVERTICALSPACING]:</strong> ' ...
...
if numel(y) < n % If there will be clipping, notify user
   warning(['RainCloudPlots:' mfilename ':ExceedsBoundaries'],...
      ['\n\t\t->\t<strong>[UIGETVERTICALSPACING]:</strong> ' ...
